function compileSB3(json, assets = {}, settings = {}){
	...
	let forceYield = false; //If, after generating IR, a yield is forced (say, by a wait block)
  
  //Generate the IR for some sets of blocks
	function generateIR(blocks){
		let res = {};

    //Generate the IR for expression blocks
...
//				argIDs = ir[j][0].argIDs;
				...
				for(const k in argNames){
					...
					if(chars[token] === "$") //To account for temporary variables
						token += 2;
				}
